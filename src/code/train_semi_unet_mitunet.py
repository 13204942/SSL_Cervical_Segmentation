...
def train(args, snapshot_path):
    ...
    model1 = Unet2D(in_chns=3, class_num=num_classes).cuda()

    model2 = model = smp.Unet(
        encoder_name='mit_b0',
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if iter_num % 500 == 0:
                ...
                cv2.imwrite(f"/root/autodl-tmp/assets_unet_mitunet/image_{iter_num}.png",np.transpose(image_save.data.cpu().numpy(),(1,2,0)))
                ...
                cv2.imwrite(f"/root/autodl-tmp/assets_unet_mitunet/outputs1_{iter_num}.png",img1_outputs.data.cpu().numpy())
                ...
                print(f'test_output shape: {test_output.shape}')
                print(f'img_tensor shape: {img_tensor.shape}')
                cv2.imwrite(f"/root/autodl-tmp/assets_unet_mitunet/outputs2_{iter_num}.png",img2_outputs.data.cpu().numpy())
                ...
                cv2.imwrite(f"/root/autodl-tmp/assets_unet_mitunet/labs_{iter_num}.png",np.transpose(labs_tensor.data.cpu().numpy(),(1,2,0)))
