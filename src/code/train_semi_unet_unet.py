    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if iter_num > 0 and iter_num % 250 == 0:
                ...
                if performance1 > best_performance1 and iter_num > 500:
                ...
                if performance2 > best_performance2 and iter_num > 500:
            ...
            if iter_num % 250 == 0:
                ...
                cv2.imwrite(f"/root/autodl-tmp/assets_unet/image_{iter_num}.png",np.transpose(image_save.data.cpu().numpy(),(1,2,0))) 
                ...
                cv2.imwrite(f"/root/autodl-tmp/assets_unet/outputs1_{iter_num}.png",img1_outputs.data.cpu().numpy())
                ...
                print(f'test_output shape: {test_output.shape}')
                print(f'img_tensor shape: {img_tensor.shape}')
                cv2.imwrite(f"/root/autodl-tmp/assets_unet/outputs2_{iter_num}.png",img2_outputs.data.cpu().numpy())
                ...
                cv2.imwrite(f"/root/autodl-tmp/assets_unet/labs_{iter_num}.png",np.transpose(labs_tensor.data.cpu().numpy(),(1,2,0)))
